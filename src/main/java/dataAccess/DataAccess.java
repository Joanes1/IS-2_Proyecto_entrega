...
public class DataAccess {
	...
	public ExtendedIterator<Event> getEvents(Date date) {
		...
		ExtendedIteratorClass extendedIt = new ExtendedIteratorClass(res);
			...
			for (Jarraitzailea reg : user.getJarraitzaileLista()) {
				...
				if (b) {
					if (erab.getNork().getDiruLimitea() < balioa) {
						this.ApustuaEgin(erab.getNork(), quote, erab.getNork().getDiruLimitea(),
	...
	public Event gertaeraKoptiatuLaguntzailea1(Event a, Date date) {
		...
		Team lokala = new Team(taldeak[0]);
		Team kanpokoa = new Team(taldeak[1]);
		Event gertKopiatu = new Event(a.getDescription(), date, lokala, kanpokoa);
		gertKopiatu.setSport(a.getSport());
		return gertKopiatu;
	}

	public void gertaeraKoptiatuLaguntzailea2(Question q, Event gertaera) {
		Question que = new Question(q.getQuestion(), q.getBetMinimum(),
				gertaera.getSport().getEvents().get(gertaera.getSport().getEvents().size() - 1));
